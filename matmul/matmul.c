...
void matmul(float *A, float *B, float *C, int M, int N, int K,
  ...
  {
    #pragma omp for private(i, jj, k, kk, v)
    for (j = 0; j < N; j += TILE_SIZE) {
      for (jj = 0; jj < TILE_SIZE; jj++) {
        for (k = 0; k < VEC_K; k++) {
          float b[VECTOR_SIZE];
          for (v = 0; v < VECTOR_SIZE; v++) b[v] = B[(k * VECTOR_SIZE + v) * N + j + jj];
          B_vecs[k * N + j + jj] = _mm512_load_ps(b);
        }
      }
        ...
        for (i = 0; i < M_per_rank; i++) {
          for (jj = 0; jj < TILE_SIZE; jj++) {
            ...
            for (kk = 0; kk < TILE_SIZE; kk++) {
              __m512 a = A_vecs[i * VEC_K + k + kk];
              __m512 b = B_vecs[(k + kk) * N + j + jj];
